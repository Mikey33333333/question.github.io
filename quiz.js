...
const traAgainBtn = document.querySelector('.traagain-btn');
...
nextBtn.onclick = () => {
   ...
   if (questionCount < questions.length - 1) {  // Use questions.length - 1
      questionCount++;
...
function showQuestion(index){
   ...
   for (let i = 0; i < option.length; i++){
      option[i].setAttribute('onclick', 'optionSelectd(this)');
   }
}

function optionSelectd(answer) {
...
function showResultBox(){
   ...
   let progressEndValue = Math.round((userScore / questions.length) * 100); // ✅ Calculate percentage
   let speed = 20; // Speed of animation

   let progress = setInterval(() => {
      if (progressStartValue >= progressEndValue) {
         clearInterval(progress); // ✅ Stop at correct percentage
      } else {
         progressStartValue++;
         progressValue.textContent = `${progressStartValue}%`;  // ✅ Show percentage
